...
class StoreRepository private constructor(
    ...
    val customers: Flow<List<CustomerAccount>> = customerDao.getAllCustomers().map { list ->
        list.map { it.toModel() }
    }

    val products: Flow<List<ProductItem>> = productDao.getAllProducts().map { list ->
        list.map { it.toModel() }
    }

    val transactions: Flow<List<TransactionItem>> = transactionDao.getAllTransactions().map { list ->
        list.map { it.toModel() }
    }
